document.addEventListener("DOMContentLoaded", function () {
  ...
  function showLightbox(projet) {
    ...
    lightboxTitle.textContent = projet.alt || "mes_projets"; // Définit le titre
  ...
  function updateLightbox() {
    const projets = getMesprojets(); // Récupère les projets actuels
    ...
    lightboxImage.src = originalSrc; // Charge l'image originale
    lightboxTitle.textContent = newProjet.alt || "mes_projets"; // Définit le titre
    lightboxCategory.textContent = category || ""; // Définit la catégorie
  }
  ...
  document.body.addEventListener("click", function (e) {
    ...
    if (icon) {
      ...
      currentProjetIndex = Array.from(projets).findIndex((projet) =>
        projet.closest(".photo-item").contains(icon)
  ...
  prevButton.addEventListener("click", function () {
    const projets = getMesprojets(); // Récupère les projets actuels
    currentProjetIndex =
      (currentProjetIndex - 1 + projets.length) % projets.length; // Reculer
    updateLightbox();
  });
